...
## Step 10: Create `src/scripts/test-liquidity.ts` — Live test script

This script runs on Railway against the real database and real Scrydex API. It tests the full liquidity pipeline with actual card data.
...
import { compositeScore, assignGrade, calculateLiquidity } from '../services/liquidity/composite.js';
import { adjustTierForLiquidity } from '../services/liquidity/tier-adjuster.js';
...
async function main() {
  ...
  check(`GRAIL + ${liquidity.grade} → ${adjusted}`, adjusted === expectedAdjustment, `expected ${expectedAdjustment}`);
...
── Test 1: Fetch a real card with pricing + trends ──
  ✅ Found card with pricing data — Charizard ex
  Card: Charizard ex (sv3pt5-006)
  Variant: holofoil

── Test 2: Tier 1 signals (from synced data) ──
  ✅ scoreTrendActivity returns 0-1 — 0.75
  ✅ scorePriceCompleteness returns 0-1 — 1
  ✅ scorePriceSpread returns 0-1 — 0.82 (NM)

── Test 3: Tier 2 signals (eBay-derived) ──
  ✅ scoreSupply(3) returns 0.6 — 0.6
  ✅ scoreSold(2) returns ~0.667 — 0.6666666666666666

── Test 4: Tier 3 velocity (real Scrydex API — 3 credits) ──
  ✅ getVelocity returned data — fetched=true
  ✅ sales7d is a number — 4
  ✅ sales30d is a number — 12
  ✅ scoreVelocity returns 0-1 — 0.85

── Test 5: Velocity cache ──
  ✅ Velocity cached in DB
  ✅ Cache has sales_7d — 4
  ✅ Cache has sales_30d — 12
  ✅ Cache has fetched_at

── Test 6: Composite liquidity score ──
  ✅ Composite score is 0-1 — 0.784
  ✅ Grade is valid — high

── Test 7: Tier adjustment with real grade ──
  ✅ GRAIL + high → GRAIL — expected GRAIL

── Test 8: Velocity endpoint ──
  ✅ Velocity endpoint returns 200 — status=200
  ✅ Response has velocity data
  ✅ Response has liquidity data
  ✅ Liquidity has grade — high

──────────────────────────────────────────────────
✅ 19 passed, ❌ 0 failed
```

This tests the full liquidity pipeline with real data: real card pricing from the database, a real Scrydex velocity API call (3 credits), real composite scoring, and the live HTTP endpoint.

---

## Step 11: Verify on Railway (manual checks)
