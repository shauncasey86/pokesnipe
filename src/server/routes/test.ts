...
router.get("/ebay", requireAuth, async (_req, res) => {
  ...
  try {
    ...
    const results = await searchItems("pokemon card", 6, { categoryId: "183454", filter: "price:[3..],buyingOptions:{FIXED_PRICE}", sort: "newlyListed" });
    const elapsed = Date.now() - start;
...
router.get("/pipeline", requireAuth, async (_req, res) => {
  ...
  try {
    ...
    // Step 1: Search eBay
    const listings = await searchItems("pokemon card", 5, { categoryId: "183454", filter: "price:[3..],buyingOptions:{FIXED_PRICE}", sort: "newlyListed" });
...
router.get("/", requireAuth, async (_req, res) => {
  ...
  for (const [name, fn] of Object.entries({
    ...
    ebay: async () => {
      const items = await searchItems("pokemon card", 1, { categoryId: "183454", filter: "price:[3..],buyingOptions:{FIXED_PRICE}", sort: "newlyListed" });
